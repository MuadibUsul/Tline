...
export function GET(request: Request) {
  ...
  const ascii = (value: string, fallback: string) => value.replace(/[^\x20-\x7E]/g, " ").replace(/\s+/g, " ").trim() || fallback;
  const title = ascii(query.get("title") || SITE_NAME, SITE_NAME).slice(0, 100);
  const eyebrow = ascii(query.get("kind") || "Institutional Intelligence", "Institutional Intelligence").slice(0, 50);
  const subtitle = ascii(query.get("subtitle") || "Research / Data / Consensus / Signal", "Research / Data / Consensus / Signal").slice(0, 140);
  return new ImageResponse(
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", padding: "72px", background: "#0c0e13", color: "#e8eaee", fontFamily: "Arial, sans-serif" }}>

...
export async function GET() {
  ...
  const items = articles.map((article) => `<item><title>${xml(article.title)}</title><link>${base}/en/research/${article.id}</link><guid isPermaLink="true">${base}/en/research/${article.id}</guid><pubDate>${article.publishedAt.toUTCString()}</pubDate><dc:creator>${xml(article.institution.name)}</dc:creator><description>${xml(article.analysis?.summary ?? article.title)}</description><source url="${xml(article.sourceUrl)}">${xml(article.institution.name)}</source></item>`).join("");

...
- **Publication gate** (`src/lib/publication.ts`): research becomes publicly readable once
  complete source text and its English PDF are ready. Reviewed Chinese text/PDF replaces the
  source view when available; otherwise Chinese mode shows the complete English original and
  never renders a processing placeholder.
